...
use crate::{font::FontInfo, query::QueryCriteria, Result, DEFAULT_BATCH_SIZE};
...
impl FontCache {
    ...
    pub fn clean_missing_fonts(&self, existing_paths: &HashSet<String>) -> Result<()> {
        ...
        for id in &missing_ids {
            tx.execute("DELETE FROM font_properties WHERE font_id = ?", params![id])?;
    ...
    fn get_connection(&self) -> Result<Connection> {
        if let Some(conn) = &self.conn {
            ...
            let tables: Vec<String> = {
                let mut stmt =
                    conn_guard.prepare("SELECT name FROM sqlite_master WHERE type='table'")?;
...
impl QueryBuilder {
    ...
    fn with_property(mut self, prop_type: &str, tags: &[String]) -> Self {
        ...
        self.where_clauses
            .push(format!("{}.value IN ({})", alias, placeholders.join(", ")));
